...
【输入】配置 `cfg`、单条任务 `item`、当前 prompt 文档、target `model` 等。
【输出】结构化任务结果（prediction、score、trajectory 等），供训练循环消费。

`EnvAdapter` 负责：
1. 使用 target 模型 + 当前 prompt 文档执行任务
...
class TemplateBenchmarkEnv(EnvAdapter):
    ...
    async def execute(self, item, prompt: str, model):
        """使用 target 模型执行单条任务。

        Args:
            item: DataItem，含 .id、.input、.ground_truth、.metadata
            prompt: 当前 prompt 文档内容（Markdown 字符串）
            model: target 模型后端实例

        Returns:
            TaskResult，含 prediction、score、trajectory
        """
        # 步骤 1：组合 prompt 与任务输入构建 prompt
        prompt = self.build_prompt(item, prompt)
    ...
    def build_prompt(self, item, prompt: str) -> str:
        """将 prompt 文档与任务输入组合。"""
        return f"{prompt}\n\n---\n\nQuestion: {item.input}"
