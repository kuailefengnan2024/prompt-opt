"""【功能描述】prompt-opt Web 界面：在浏览器中配置、启动并监控训练。
【输入】命令行参数（--port、--host、--share）、YAML 配置与超参覆盖。
【输出】Gradio 服务与训练子进程日志/进度展示。

用法:
    python -m promptopt_webui.app [--port PORT] [--share]
"""
import argparse
import glob
import json
...
class TrainingManager:
    ...
    def _parse_stage(self, line: str):
        ...
        elif "meta prompt" in line_lower:
            self.stage = "🧠 Meta Prompt"
    ...
    def get_colored_logs_html(self) -> str:
        ...
        parts = []
        for line in lines:
            # 品牌替换：日志中显示 promptopt 而非 reflact
            line_display = line.replace("reflact", "promptopt").replace("Reflact", "Promptopt").replace("REFLACT", "PROMPTOPT")
...
def build_ui():
    ...
    with gr.Blocks(
        title="prompt-opt WebUI",
    ) as app:
        gr.Markdown("# 🧠 prompt-opt Training Dashboard")
        gr.Markdown("*PROMPTOPT: Executive Strategy for Self-Evolving T2I Prompts — Configure, launch, and monitor training.*")
...
def main():
    parser = argparse.ArgumentParser(description="prompt-opt WebUI")
