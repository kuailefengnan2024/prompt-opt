"""【功能描述】Reflect 环境适配器抽象接口：接入新 benchmark/模拟器时实现 `EnvAdapter` 子类。
【输入】扁平化 `cfg`、批次 `BatchSpec`、当前 prompt 文档、rollout 输出等。
...
from promptopt.datasets.base import BaseDataLoader, BatchSpec
from promptopt.llm_templates import load_template


class EnvAdapter(ABC):
    """将 Reflect 接入任意环境的抽象适配器。

    子类必须实现全部抽象方法；Reflect trainer 在流水线各阶段调用它们。
    """
    ...
    def rollout(
        ...
        **kwargs,
        ...
        -------
        list[dict]
            每个 dict 符合 :class:`~promptopt.types.RolloutResult`：
    ...
    def reflect(
        ...
        **kwargs,
        ...
        每个返回 dict 符合 :class:`~promptopt.types.RawPatch`：
    ...
    #
    # 通过 ``load_template(name, env)`` 从 ``.md`` 加载：
    #   1. ``promptopt/envs/<env>/prompts/<name>.md``  （环境专属）
    #   2. ``promptopt/llm_templates/<name>.md``             （通用回退）
    #
    ...
    def _env_name(self) -> str:
        """从本适配器模块路径推导 env 目录名。"""
        # 例: "promptopt.envs.searchqa.adapter" → "searchqa"
    ...
    def _load_env_prompt(self, name: str) -> str | None:
        ...
        try:
            return load_template(name, env=self._env_name)
    ...
    def get_error_minibatch_prompt(self) -> str | None:
        ...
        if raw_mode in {"full_rewrite", "full_rewrite_minibatch", "minibatch_full_rewrite", "prompt_rewrite_minibatch"}:
