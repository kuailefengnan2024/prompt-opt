"""【功能描述】Reflect 慢更新 — epoch 级纵向 prompt 精炼；每 epoch 结束时比较相同样本集在上一 epoch prompt 与当前 epoch prompt 下的 rollout 表现，由 optimizer 分析回归与持续失败，并将指导写入 prompt 受保护区域。
...
def run_slow_update(
    ...
    *,
    ...
    ----------
    prompt_content : str
        当前 epoch 的 prompt（fast update 之后）。
    results_prev : list[dict]
        20 个样本在上一 epoch prompt 下的 rollout 结果。
    results_curr : list[dict]
        20 个样本在当前 epoch prompt 下的 rollout 结果。
    items : list[dict]
        用于比较的 20 个样本 item。
    prev_prompt : str
        上一 epoch 的 prompt 内容。
    prev_slow_update_content : str
    ...
    -------
    dict | None
        符合 :class:`~promptopt.types.SlowUpdateResult`：
        ``{"reasoning": str, "slow_update_content": str}`` 或 ``None``。
    """
    actual_system = system_prompt if system_prompt is not None else load_template("slow_update")
    ...
    user = (
        f"## Previous Epoch's Prompt\n{prev_prompt_display}\n\n"
        f"## Current Epoch's Prompt\n{prompt_content}\n\n"
        ...
        f"## Longitudinal Comparison (same 20 tasks, two prompt versions)\n"
        f"{comparison_text}"
    )
