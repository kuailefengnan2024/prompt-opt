"""【功能描述】Reflect 训练器，主编排 6 阶段 Reflect 流水线。
...
训练器与环境无关；环境相关逻辑均委托给
:class:`~promptopt.envs.base.EnvAdapter`。
"""
...
from promptopt.datasets.base import BatchSpec
from promptopt.envs.base import EnvAdapter
from promptopt.evaluation.gate import evaluate_gate
from promptopt.gradient.aggregate import merge_patches
from promptopt.optimizer.meta_prompt import run_meta_prompt
from promptopt.optimizer.clip import rank_and_select
from promptopt.optimizer.lr_autonomous import decide_autonomous_learning_rate
from promptopt.optimizer.rewrite import rewrite_prompt_from_suggestions
from promptopt.optimizer.scheduler import build_scheduler
from promptopt.optimizer.prompt_editor import apply_patch_with_report
from promptopt.optimizer.slow_update import (
...
from promptopt.utils import compute_score, prompt_hash
...
def _best_prompt_path(out_root: str, art: PromptArtifacts) -> str:
    return os.path.join(out_root, art.best_file)
...
def _normalise_patches(
    ...
    每个元素应符合 :class:`~promptopt.types.RawPatch`。
    """
...
def _load_template(out_root: str, step: int, art: PromptArtifacts | None = None) -> str:
    layout = art or PromptArtifacts()
...
def _load_meta_prompt_content(out_root: str, epoch: int) -> str:
    if epoch <= 0:
        return ""
    path = os.path.join(
        out_root, "meta_prompt", f"epoch_{epoch:02d}", "meta_prompt_result.json",
    )
    ...
    try:
        with open(path) as f:
            result = json.load(f)
        return str(result.get("meta_prompt_content", "")).strip()
    except Exception:
        return ""
...
class ReflectTrainer:
    ...
    def train(self) -> dict:
        """执行完整 Reflect 训练循环，返回 summary 字典。"""
        cfg = self.cfg
        ...
        if runtime_state:
            ...
            current_prompt = _load_template(out_root, last_step, art)
            ...
            if os.path.exists(best_prompt_path):
                ...
                best_prompt = _load_template(out_root, best_step, art)
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            if use_slow:
                ...
                if os.path.exists(slow_done_path):
                    ...
                    prev_prompt = _load_template(out_root, prev_epoch_last_step, art)
            ...
            if use_meta_prompt:
                ...
                if os.path.exists(meta_prompt_done_path):
                    print(f"\n  [META PROMPT epoch {epoch}] resumed — already done")
                elif epoch == 1:
                    ...
                    print(f"\n  [META PROMPT epoch {epoch}] skipped — first epoch")
                else:
                    print(
                        f"\n  {'='*60}\n"
                        f"  META PROMPT — Epoch {epoch} "
