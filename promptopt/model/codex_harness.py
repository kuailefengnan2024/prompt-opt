...
【输入】工作区路径、prompt/task 文本、prompt、模型与超时、后端配置。
...
from promptopt.model.backend_config import (
...
def render_prompt_md(
    prompt_content: str,
    *,
    name: str = "promptopt-target",
    description: str = "Dynamic Reflect prompt for the current benchmark task.",
    preamble: str = "",
) -> str:
...
def prepare_workspace(
    *,
    ...
    os.makedirs(os.path.join(work_dir, ".agents", "prompts", "promptopt-target"), exist_ok=True)

    prompt_path = os.path.join(work_dir, ".agents", "prompts", "promptopt-target", "TARGET_PROMPT.md")
...
def _build_codex_trace_summary(raw: str, response: str) -> str:
    ...
    parts.append(f"- read TARGET_PROMPT.md: {prompt_read}")
    if exec_errors:
...
def _exec_prompt(prompt: str, *, allow_file_edits: bool = False) -> str:
    ...
    return (
        "Use the workspace files to solve the task. Read task.md and the prompt at "
        ".agents/prompts/promptopt-target/TARGET_PROMPT.md before answering. "
        "If ATTACHMENTS.md exists, read it and inspect the listed local files. "
        "Do not call a Skill tool; the Reflect guidance is a local markdown file. "
...
def _retry_prompt(prompt: str, attempt: int) -> str:
    ...
    return (
        ...
        ".agents/prompts/promptopt-target/TARGET_PROMPT.md. If ATTACHMENTS.md exists, use the listed files. "
...
def _normalize_target_exec_prompt(prompt: str) -> str:
    ...
    text = prompt or ""
    replacements = {
        "Use the `promptopt-target` prompt available in this workspace.": (
            "Read `.agents/prompts/promptopt-target/TARGET_PROMPT.md` directly; do not call a Prompt tool."
        ),
        "- Use the local `promptopt-target` prompt before writing code.": (
            "- Read `.agents/prompts/promptopt-target/TARGET_PROMPT.md` before writing code; do not call a Prompt tool."
        ),
    }
...
def _run_codex_cli_exec(
    *,
    ...
    try:
        ...
        raise
    try:
        from promptopt.model import azure_openai as _openai
