"""【功能描述】验证门（validation gate）：接受或拒绝候选 prompt，类比 NN 验证早停与模型选择。
【输入】候选 prompt、候选 hard 分、当前/历史最优 prompt 与分数及 `global_step`。
...
def evaluate_gate(
    ...
    if cand_hard > current_score:
        new_current_prompt = candidate_prompt
        new_current_score = cand_hard
        if cand_hard > best_score:
            return GateResult(
                action="accept_new_best",
                current_prompt=new_current_prompt,
