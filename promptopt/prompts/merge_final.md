...
1. **FAILURE PATCHES TAKE PRIORITY**：除非与 success 直接冲突，否则保留 failure edits。
2. **Deduplicate**：同一点冲突时保留 failure version。
3. **Preserve success insights**：纳入 failure 尚未覆盖的 success edits。
4. 每条 edit 携带 support_count 和 source_type。
